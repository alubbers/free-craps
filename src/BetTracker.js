...
class BetTracker {
  ...
  getBetForBucketCode(bucketCode) {
    return this.bets.find((b) => b.bucketCode === bucketCode);
  }
  ...
  makeBet(amount, groupCode, option = "default") {
    ...
    let existingBet = this.bets.find(
      (b) => b.groupCode === groupCode && b.option === option,
    );
   ...
   */
  replaceBets(newBets) {
    const removedBets = this.bets.filter(
      (b) => !newBets.some((newBet) => b.bucketCode === newBet.bucketCode),
    );

    removedBets.forEach((removed) => this.clearBet(removed.bucketCode));

    newBets.forEach((newBet) =>
      this.makeBet(newBet.amount, newBet.groupCode, newBet.option),
    );
  }
